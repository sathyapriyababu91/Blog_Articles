...
function BlogDetails() {
  ...
  const fetchSingleBlog = async () => {
    setLoading(true);
    try {
      // 1. Try fetching from backend database first
      const res = await axios.get(`http://localhost:8080/api/blogs/${id}`);
      if (res.data) {
        setBlog(res.data);
      }
    } catch (err) {
      // 2. Fallback to static blogs if backend ID isn't found
      const foundStatic = staticBlogs.find((b) => b.id === id);
      if (foundStatic) {
        setBlog(foundStatic);
      }
    } finally {
      // Fetch related blogs list for the sidebar
      const allStaticRelated = staticBlogs.filter((b) => b.id !== id);
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#070A13] flex justify-center items-center">
        <p className="text-cyan-400 font-bold animate-pulse">Loading Article Details...</p>
      </div>
    );
  }
  ...
  if (!blog) {
    return (
      <div className="p-20 text-center bg-[#070A13] min-h-screen flex flex-col justify-center items-center">
        <p className="text-xl font-bold text-gray-500">😢 Article not found</p>
        <button 
          onClick={() => navigate('/AI_Blog_Articles/blog')} 
        ...
        <div className="lg:col-span-2 bg-[#0B0F19]/65 backdrop-blur-xl rounded-3xl border border-gray-800/80 shadow-2xl p-6 md:p-8 flex flex-col justify-between">
          <div>
            <div className="relative aspect-video w-full overflow-hidden rounded-2xl bg-[#070A13] border border-gray-800/50 shadow-inner">
              <img
                src={blog.image || "https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?w=800"}
            ...
            <h1 className="text-2xl md:text-4xl font-black text-white mt-6 leading-tight tracking-tight uppercase text-cyan-400">
              {blog.title}
            </h1>
            ...
            <div className="flex items-center gap-3 my-6 p-4 bg-[#121826]/40 rounded-2xl border border-gray-800/60">
              ...
              <div>
                <p className="text-sm font-extrabold text-gray-200">
                  {blog.author || blog.username || "Admin"}
                </p>
          ...
          <div className="flex items-center justify-between gap-4 mt-10 pt-6 border-t border-gray-900">
            ...
            >
              ...
              <span className="ml-1 px-1.5 py-0.5 rounded-md text-[10px] font-black bg-gray-800">{count}</span>
            </button>
        ...
        <div className="bg-[#0B0F19]/60 backdrop-blur-xl rounded-3xl border border-gray-800/80 shadow-2xl p-6 h-fit sticky top-6">
          ...
          <div className="space-y-4">
            {relatedBlogs.slice(0, 4).map((item) => (
              <div
                key={item.id}
                onClick={() => navigate(`/AI_Blog_Articles/blog/${item.id}`)}
              ...
              >
                <div className="w-20 h-16 rounded-xl overflow-hidden bg-[#070A13] border border-gray-900 flex-shrink-0">
                  <img src={item.image} className="w-full h-full object-cover opacity-70 group-hover:opacity-95" alt="" />
                </div>
